...
function buildPrompt({ layersMartinList, layersMartinCount, starsOptgeoList, starsOptgeoCount, starsOptgeoStyleIds }) {
  return `# CHUKEI_PROMPT.md

システムプロンプトは保存できるがインターネットに一切アクセスできない生成AI「源内」(Digital Agency運用、AWS Generative AI Use Cases OSS)向けの、単独で完結するStaffプロンプト。\`dwg7/spiccato\`の[GENNAI_PROMPT.md](https://github.com/dwg7/spiccato/blob/main/GENNAI_PROMPT.md)を直接の親として、応答フォーマットのみを差し替えた狭いフォーク(\`DECISIONS.md\` D6)。それ以外の内容(カタログ埋め込み、\`source_id\`/\`style_id\`捏造禁止、bbox推測ポリシー、2カタログ構成、Cartographer挙動の説明)はGENNAI_PROMPT.mdからそのまま継承している。

北海道地方測量部の職員がGennai経由で地図を求める、この一つのデプロイのために作られている(\`CLAUDE.md\`参照)。

**このファイルは自動生成される**(\`scripts/build-chukei-prompt.mjs\`、\`npm run build\`で毎回、layers-martin/stars-optgeoの実カタログから再生成)。手で編集しないこと。生成日時はこのファイル自体には埋め込まない(diffノイズを避けるため) — 最新版は常にこのリポジトリの\`main\`ブランチを参照すること。
