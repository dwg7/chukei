...
`CHUKEI_COLLABORATION_PROMPT.md` (`DECISIONS.md` D31) is the same idea with a partnership/outreach-planning layer added in front — for staff designing collaborations (school lessons, disaster-prep workshops, industry study sessions, etc.) rather than just asking for a single map. Built by `scripts/build-chukei-collaboration-prompt.mjs` (`npm run build:collaboration`), which embeds `CHUKEI_PROMPT.md`'s current on-disk content verbatim — **run `npm run build` first** so it's not embedding something stale.
