...
4. **わざと壊す**: このカタログに存在しない/曖昧すぎる質問をぶつけて、「見つかりませんでした」の terse な応答を見せる。**ここが今日のデモの肝** — 失敗しても静かに、変な言い訳もせず終わる様子を見せることで、「壊しても大丈夫」を言葉でなく体験させる。この応答にもフィードバックリンクが付くので、次項の説明にそのまま繋げられる。

**フィードバックフォームは別デモ不要**(2026-08-24、GitHub issue #1、`DECISIONS.md` D21): 応答フォーマットが変わり、`[フィードバックする]`リンクが**すべての応答に自動で付く**ようになった(成功時・見つからない場合とも)。1〜4のどのデモでも自然に画面に出るので、「フィードバックフォームへの導線」として別立てのデモ手順は要らない。むしろ「見てください、この応答についているこのリンクからワンクリックでフィードバックできます」と1〜4のどこかで一言添えるだけで済む — 余った時間は質疑に回してよい。
...
A: 応答に必ず付いている「フィードバックする」リンクを開いて、Chukeiの回答をそのまま貼って教えてほしい。これはパイロットで、そのために作った仕組み(`FEEDBACK_FORM.md`)。リンクは2026-08-24から全応答(成功時・見つからない場合とも)に自動で付くようになった。

**Q: 「近い地図」って何?(応答をよく見ている人向け)**
A: 要求にそのまま合うデータが無く、周辺データや近い年代・近い主題図などでベストエフォート対応した場合、リンクの手前に「近い」と付く(例:「近い[地図](...)を用意しました。」)。理由までは書かないが、「これは完全一致ではないかもしれない」というサインなので、そのつもりで見てほしい。2026-08-24追加(GitHub issue #2、`DECISIONS.md` D22)。デモの4つのクエリはいずれも直接一致するので「近い」は付かない — もし本番で試して「近い」付きの応答を見た人がいたら、それも立派なフィードバック。
